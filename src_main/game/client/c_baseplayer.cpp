...
void C_BasePlayer::PostDataUpdate( DataUpdateType_t updateType )
{
	...
	//  Tsai 20180913 :这必须发生在这个函数中而不是OnDataChanged中，
	//  这样同一帧中创建的指向player的EHandles不会被盖戳，因为预测认为还没有一个本地玩家
	if ( updateType == DATA_UPDATE_CREATED )
	{
		// Make sure s_pLocalPlayer is correct  
		//  Tsai 20180913 : 保证s_pLocalPlayer是正确的
		//  int      m_nPlayerSlot;	// slot of HLTV client on game server
		//  int CEngineClient::GetLocalPlayer( void )
		//  {
		//     return cl.m_nPlayerSlot + 1;
		//  }
		//  但是按照这个写法，感觉PostDataUpdate这个函数执行的次数是一次啊。因为在引擎端是累加的，一个玩家应该就只加一次。
		//  这里的engine变量在client就是CEngineClient
		...
		{
			// Tsai 20180913 : 从这个判断上希望s_pLocalPlayer是null			
			...
			// Tsai 20180913 : 重置混合的声音，以防我们在更改级别时摄像机被冻结（停止），这（我猜是指的重置混合的声音这个事情）将导致snd_soundmixer被修改。
...
// Output : C_BasePlayer
// Tsai 20180913: 在最开始应该是null的，当C_BasePlayer::PostDataUpdate调用时预测没有本地玩家，然后才有了s_pLocalPlayer = this
//-----------------------------------------------------------------------------
