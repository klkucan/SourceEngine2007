...
// Tasi 20180913: 不要修改布局 和大小，保持与HL2_VC6的兼容性。

class CBaseEdict
{
public:
	// Tasi 20180913: 返回一个IServerEntity，如果FL_FULLEDICT设置。 或者返回null，如果这一个轻量的网络entity。
...
public:
	...
	// NOTE: this is in the edict instead of being accessed by a virtual because the engine needs fast access to it.
	int m_NetworkSerialNumber;	// Game DLL sets this when it gets a serial number for its EHANDLE.  GameDll设置这个值，当它获得一个EHANDLE的序列数字
...
//  of the structure
//
//  Tsai 20180913: 引擎内部entity的代表，包含一些基础的碰撞和位置信息，以及
//   一个指向类（被包装在架构上层）的指针。
//-----------------------------------------------------------------------------
